...
if(isset($_POST['create'])){
	...
	$existingEmailResult = mysqli_query($con, "select * from hospital where email='$email';");
	...
	if($row = mysqli_fetch_assoc($existingEmailResult)){
		...
		mysqli_query($con, "insert into hospital(name, city, detailed_address, email, password, about) 
			values('$name', '$city', '$detailed_address', '$email', '$new_pass', '$about');");
	}
	
	// go to profile
	if($data_error == ""){
		$result = mysqli_query($con, "select id from hospital where email = '$email' and password='$new_pass'");
